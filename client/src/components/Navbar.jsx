    ...
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all z-50">
        ...
        {!user ? (
          ...
          <div className="relative group border-[3px] rounded-full border-gray-600 cursor-pointer">
            ...
            <ul className="hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 w-36 rounded-md text-sm z-40">
              {/* If user is a FARMER or ADMIN, show dashboard link */}
              {(user?.role === "FARMER" || user?.role === "ADMIN") && (
                <li
                  onClick={() => navigate("/seller")}
                  className="p-1.5 pl-3 hover:bg-primary/20 cursor-pointer font-medium text-primary-dull"
                >
                  Farm Dashboard
                </li>
              )}
      ...
      {open && (
        ...
        >
          ...
          {/* Mobile Farm Dashboard link for FARMER/ADMIN */}
          {user && (user?.role === "FARMER" || user?.role === "ADMIN") && (
            <NavLink
              to="/seller"
              onClick={() => setOpen(false)}
              className="font-medium text-primary-dull"
            >
              Farm Dashboard
            </NavLink>
          )}
